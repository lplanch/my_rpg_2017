...
void draw_fast_inv_slot(st_rpg *rpg)
{
	...
	if (rpg->finv.slots[0] == NULL) {
		sfSprite_setPosition(rpg->inv.highlight, (sfVector2f){pos.x + 596, pos.y + 230});
		sfRenderWindow_drawSprite(rpg->window, rpg->inv.highlight, NULL);
	} if (rpg->finv.slots[1] == NULL) {
		sfSprite_setPosition(rpg->inv.highlight, (sfVector2f){pos.x + 473, pos.y + 135});
		sfRenderWindow_drawSprite(rpg->window, rpg->inv.highlight, NULL);
	} if (rpg->finv.slots[2] == NULL) {
		sfSprite_setPosition(rpg->inv.highlight, (sfVector2f){pos.x + 350, pos.y + 230});
		sfRenderWindow_drawSprite(rpg->window, rpg->inv.highlight, NULL);
	}
	verify_fast_inv_slots_draw(rpg);
}
